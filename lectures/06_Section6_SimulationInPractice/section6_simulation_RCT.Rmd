---
title: "Simulation Study: Example 2 - Covariate Adjustment in an RCT"
...
Let's take a look at another question we can answer with simulation: comparing adjusted to unadjusted estimates of the ITT effect in a randomized trial. Again, it would help us to get more specific with our intentions. For instance, we might ask whether adjusting for known causes of the outcome yields an improvement in the bias and efficiency of the ITT estimator. 
...
In terms of generating our data, we will rely on a simple DAG, with a set of prognostic covariates (causes of the outcome, but — because treatment is randomized — not of the exposure, so they are not confounders), a single exposure, and a single outcome:

```{r, out.width = "10cm",fig.cap="Simple causal diagram representing a randomized trial, with randomized treatment assignment $A$, outcome cause $C$, and outcome $Y$.",echo=F}
...
Once the true values are obtained, we can then run our simulation. Rather than a single scenario, we'll run a full factorial of scenarios: three sample sizes (250, 500, and 1000), three covariate counts (5, 10, and 15), and two covariate correlation structures (independent covariates, and a pairwise correlation of 0.5), with 10,000 simulated datasets in each of the 18 scenarios. Let's look through the code we'll use to do this:
...
If our outcome was simulated from a nonlinear model, such as logistic regression, we'd have to take into consideration noncollapsibility here, which would add a considerable degree of complexity to our simulation study.
