...
$$\logit P(\bullet) = \log \left [ \frac{P(\bullet)}{1 - P(\bullet)} \right ]$$
...
Note that this code simulates a random variable $X$ from the Bernoulli distribution (`rbinom` with `size = 1`), where `prob` is defined as the inverse of the logit of the regression model. We can write this as:
...
This $P(X = 1 \mid Z)$ is the propensity score. It's what we used in the `prob = ` argument of the `rbinom` function, and it's what we estimated when we fit a `glm` regressing our exposure against $Z$, appended with the `$fitted.values` operator.
...
The last two topics in this lecture are marginal standardization and inverse probability weighting — the two estimators whose performance we will formally evaluate in a full simulation study in Section 6 (Example 2). Our goal here is simply to understand how each one is implemented; keep them both in mind, as they will return throughout the course.
...
Two details of the `predict()` calls above are worth flagging, since we will rely on them repeatedly. First, `transform(a, x=1)` hands `predict()` a copy of the dataset in which *everyone's* exposure is set to 1 (and, likewise, `x=0` sets everyone's exposure to 0). This is what makes the predictions counterfactual rather than observed. Second, `type="response"` requests predictions on the scale of the outcome itself. With the identity link used here it makes no difference, but when the outcome model is a logistic regression, `type="link"` would return predictions on the log-odds scale, while `type="response"` returns them on the probability scale — and it is the probability-scale predictions we need to average.
