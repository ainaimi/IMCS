...
```{r, eval = FALSE, purl = FALSE, tidy = FALSE}
...
The function outputs five items: the first is the indexing argument; the second is the sample size; the third is the odds ratio parameter; the fourth is the mean of the outcome `y`, which will depend on the intercept value and the odds ratio parameter; the fifth is the estimated log-odds ratio for the association between `c` and `y`. Here is the function:
...
In my experience, `sapply` is sometimes a slower function, possibly because it builds a set of results along columns as opposed to a list.
...
## Random Number Generation with $\pi$
...
To understand seeds, it is important to understand the idea of a pseudo random number generator. A very basic (though somewhat misleading) way to explain the idea behind pseudo-random number generation is to find a sequence of digits with no apparent pattern or regularities to them. For example, if we can establish that the sequence of digits in the constant $\pi$ doesn't have any regular patterns to it, we can use it as our base to generate randomness:

$$3.14159265358979 \cdots$$
Here, the so-called "pseudo-randomness" occurs in the transition from one number in the sequence to the next. For example, if we start the random number sequence at the third digit (4), the next numbers in the sequence will be 1, then 5, then 9, and so on. Since there is no obvious predictability or regularity in this sequence, we can use these digits as the base to initialize a range of complex functions that mimic random behavior. 
...
Consider a scenario where we'd like to generate a set of probabilities (i.e., numbers bounded between 0 and 1) that are "random" using the sequence of digits in $\pi$. We can construct an algorithm such as:

- Pick a seed value, which corresponds to a digit in the sequence of numbers of $\pi$
- Select the three consecutive digits of $\pi$ after the seed, and divide these three digits by 1000
- Return the value as output
- Move the seed forward by one digit position in $\pi$, and repeat the procedure

If we did this, we'd get a sequence of "random" probabilities that are completely reproducible, and that look like this:

| Iteration | Seed | Digits Used | Output (Random Number) |
| ------: | :--: | :---------- | :--------------------- |
|       1 |   3  | 1 5 9       | 0.159                  |
|       2 |   4  | 5 9 2       | 0.592                  |
|       3 |   5  | 9 2 6       | 0.926                  |
...
This example represents the core idea of a seed, and sets the stage for understanding some of the negative consequences that we can encounter using seed values. However, this is not how pseudo random number generators work in reality. 
  ...
  <!-- runif(1)         # advances the PRNG -->
  <!-- print(.Random.seed[1:10])  # state changed -->
  <!-- ``` -->


# Using Seeds in R

Near the beginning of this pdf document, we set a seed value to demonstrate the use of for loops and the apply family for a simple simulation study. We used `set.seed(123)` to do this. Because every code chunk in this document runs in the same R session, that seed deterministically affects the random number stream in all subsequent chunks, so let's take the opportunity to reset it here:
...
Now, every time we run this code chunk, we get exactly the same values for `a` and `b`, which are 0.2875775 0.7883051 0.4089769 0.8830174 0.9404673 for `a` and 0 2 4 2 2 for `b`.
...
## Using the .Random.seed State Object

Using the `set.seed()` function properly is an important tool to enable reproducibilty of simulation results. However, suppose you run a complicated simulation study with several tens or hundreds of thousands of iterations, and weeks to run. You may want to enable yourself and the research community to generate exactly any one of the tens or hundreds of thousands of sets of results. For example, suppose you run a simulation with 100,000 Monte Carlo runs, and you are interested in reproducing the exact set of results that came from simulation number 99,673. This situation might arise if you find that there are outliers or anomalous results in this particular Monte Carlo run, and that you need to investigate further why this may be the case. 

If you only rely on the `set.seed()` function initialized once at the beginning of the simulation, you'd have to wait, possibly weeks, to get to this particular Monte Carlo number before you were able to reproduce your results, and then you would have to know beforehand what code you'd need to write to explore and potentially debug the problem. This is not a tenable way to solve this problem. 

Instead, you can rely on the `.Random.seed` object, which represents the state of the pseudo-random number generator at every particular instance during the simulation. Here's how this object works in R:
...
The `.Random.seed` object contains information on three important things. The first element in the list encodes which algorithms R is currently using, one code per setting reported by the `RNGkind()` function. In the number 10403, the last two digits (03) identify the random number generator itself: the Mersenne-Twister, R's default. The middle two digits (04) identify the method used to turn uniform draws into normal deviates ("Inversion", also the default), and the leading 1 identifies the method used for discrete uniform sampling ("Rejection"). Note that "Inversion" is not a kind of random number generator; it is a separate setting for generating normal random variables from the generator's output.

Let's skip the second element in the list for now which, in this instance, is 624. The string of numbers from the third to the 626th number represent the state of the pseudo random number generator, in this case, the Mersenne-Twister. The Mersenne-Twister is a complex algorithm that can be used to generate pseudo-random numbers. If we lean on our $\pi$ analogy, the string of numbers from the third to the 626th numbers can be thought of as the sequence of digits of $pi$, and these are what are used to generate pseudo-randomness. These numbers come from a mathematical operation (the "Twister" part) performed on a particular number that is one of the Mersenne primes (the number is $2^{19937} - 1$).

The value of the seed used in the `set.seed()` function initializes the first number in the set of 624 numbers. All subsequent numbers are then obtained deterministically using the complex Mersenne-Twister equation.
...
Effectively, with every call to the random number generator in R (in this case, `runif()`), a new seed state is initialized with the Mersenne-Twister. 

Thus, if we want to recreate exactly the random number generated by R, we have to save the seed state for the iteration that we wish to replicate:
