...
To understand seeds, it is important to understand the idea of a pseudo random number generator. A very basic (but perhaps slightly misleading) way to explain the idea behind pseudo-random number generation is to find a sequence of digits with no apparent pattern or regularities to them. For example, if we can establish that the sequence of digits in the constant $\pi$ doesn't have any regular patterns to it, we can use it as our base to generate randomness:

$$3.14159265358979 \cdots$$
Here, the so-called "pseudo-randomness" occurs in the transition from one number in the sequence to the next. For example, if we start the random number sequence at the third digit (4), the next numbers in the sequence will be 1, then 5, then 9, and so on. 
...
To make the illustration a bit more concrete, consider a scenario where we'd like to generate a set of probabilities (i.e., numbers bounded between 0 and 1) that are "random" using the sequence of digits in $\pi$. We can construct an algorithm such as:

- Pick a seed value, which corresponds to a digit in the sequence of numbers of $\pi$
- Select the three consecutive digits of $\pi$ after seed, and divide these three digits by 1000
- Return the value as output
- Move the seed forward by one digit position in $\pi$, and repeat the procedure

If we did this, we'd get a sequence of "random" probabilities that look like this:
...
However, this procedure is entirely reproducible.
